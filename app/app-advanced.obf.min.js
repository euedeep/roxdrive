...
function gdidecode(str) {
  ...
  const gdijsorg_0x22bf03 = gdijsorg_0x47d3;
  (function(_0x2015a9, _0x2d2e6f) {
    const _0x194131 = gdijsorg_0x47d3;
    while ([]) {
      try {
        const _0x50490c =
  ...
  function gdijsorg_0x47d3(_0x4aefd5, _0x2d1551) {
    _0x4aefd5 -= 0x15e;
    const _0x557938 = gdijsorg_0x5579[_0x4aefd5];
    return _0x557938;
  }
  return decodeURIComponent(
    atob(str)
      .split('')
      [gdijsorg_0x22bf03(0x16b)](_0x1cdc7a => {
        const _0x416153 = gdijsorg_0x22bf03;
...
function requestListPath(path, params, resultCallback, authErrorCallback) {
  ...
  $.post(path, p, (data, status) => {
    const res = jQuery.parseJSON(gdidecode(read(data)));
...
function list(path) {
  ...
  const breadbar = '';
  const navarrayde = decodeURIComponent(navfulllink).split('/');
  ...
  requestListPath(
    ...
    path => {
      $('#spinner').remove();
      const pass = prompt('Directory encryption, please enter the password', '');
...
function append_files_to_list(path, files) {
  ...
  const is_lastpage_loaded = $list.data('nextPageToken') === null;
  const is_firstpage = $list.data('curPageIndex') == '0';
  ...
  for (i in files) {
    ...
    if (item.mimeType == 'application/vnd.google-apps.folder') {
      html += `<a href="${p}" style="color: ${UI.folder_text_color};" class="list-group-item list-group-item-action">
      ...
      ${item.name} ${UI.display_time ? `<span class="badge bg-info float-end"> ${item.modifiedTime} </span>` : ''}</a>`;
    } else {
      const epn = item.name;
    ...
    if (item.mimeType === 'application/vnd.google-apps.folder') {
      html += `<a style="color: ${UI.folder_text_color};" ${
        UI.search_all_drives
          ...
          : `onclick="onSearchResultItemClick(this)" data-bs-toggle="modal" data-bs-target="#SearchModel" id="${item.id}"`
      ...
      var ext = item.name
        ...
        .pop()
        .toLowerCase();
      if ('|html|php|css|go|java|js|json|txt|sh|md|mp4|webm|avi|bmp|jpg|jpeg|png|gif|m4a|mp3|flac|wav|ogg|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|srt|bat|'.indexOf(`|${ext}|`) >= 0) {
        p = p + '?a=view';
        c = c + 'view';
      }
...
function onSearchResultItemClick(a_ele) {
  const me = $(a_ele);
  const teamDriveId = a_ele.getAttribute('teamDriveId');
  ...
  let content = `<div class="d-flex justify-content-center"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status"><span class="sr-only"></span></div>`;
  ...
  $.post(
    ...
    data => {
      if (data) {
        const href = `${searchhost}:${data}${can_preview ? '?a=view' : ''}`;
        if (href.endsWith('/')) {
...
function file(path) {
  ...
  const ext = name
    ...
    .toLowerCase();
  $('#content').html(
    `<div class="d-flex justify-content-center" style="height: 150px"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status"><span class="sr-only"></span></div></div>`
  );
...
function file_others(path) {
  ...
  const ext = name
    ...
    .toLowerCase();
  var path = path;
  const url = UI.second_domain_for_dl ? UI.downloaddomain + path : window.location.origin + path;
  $.post('', data => {
    const obj = jQuery.parseJSON(gdidecode(read(data)));
...
function file_code(path) {
  ...
  $.post('', data => {
    ...
    const size = formatFileSize(obj.size);
    const content = `
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/prism/1.24.1/themes/prism-tomorrow.min.css" integrity="sha256-Rl83wx+fN2p2ioYpdvpWxuhAbxj+/7IwaZrKQBu/KQE=" crossorigin="anonymous">
  <div class="container"><br>
  <div class="card text-center">
  <div class="card-body text-center">
    <div class="${UI.file_view_alert_class}" id="file_details" role="alert">${obj.name}<br>${size}</div>
  ...
  } class="line-numbers language-markup" data-src="plugins/line-numbers/index.html" data-start="-5" style="white-space: pre-wrap; counter-reset: linenumber -6;" data-src-status="loaded" tabindex="0"><code id="editor" class="lang-${code_type}"></code></pre>
  </div>
  </div>
  <div class="card-body">
  <div class="input-group mb-4">
  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/prism/1.24.1/prism.min.js" integrity="sha256-fZOd7N/oofoKcO92RzxvC0wMm+EvsKyRT4nmcmQbgzU=" crossorigin="anonymous"></script>
  `;
    $('#content').html(content);
  });  
}
...
function file_video(path) {
  const name = path.split('/').pop();
  const decodename = unescape(name);
  const caption = name.slice(0, name.lastIndexOf('.'));
  var path = path;
  const url = UI.second_domain_for_dl ? UI.downloaddomain + path : window.location.origin + path;
...
function file_image(path) {
  ...
  if (target_children) {
    ...
    if (target_children.length > 0 && target_children.includes(path)) {
      const len = target_children.length;
      const cur = target_children.indexOf(path);
      // console.log(`len = ${len}`)
      // console.log(`cur = ${cur}`)
      const prev_child = cur - 1 > -1 ? target_children[cur - 1] : null;
      const next_child = cur + 1 < len ? target_children[cur + 1] : null;
      if (prev_child == null) {
        var prevchild = false;
...
function utc2delhi(utc_datetime) {
  ...
  return `${year}-${month.substring(month.length - 2, month.length)}-${date.substring(date.length - 2, date.length)} ${hour.substring(hour.length - 2, hour.length)}:${minute.substring(
    minute.length - 2,
    minute.length
...
function read(str) {
  ...
  const gdijsorg_0x570bf1 = gdijsorg_0x158f;

  function gdijsorg_0x158f(_0x32bcea, _0x29ebfd) {
    _0x32bcea -= 0x150;
    const _0x1207c1 = gdijsorg_0x1207[_0x32bcea];
    return _0x1207c1;
  }
  (function(_0xbbe83c, _0xbbffd8) {
    const _0x2feec5 = gdijsorg_0x158f;
    while ([]) {
      try {
        const _0x5d3639 =
  ...
  const sa = str[gdijsorg_0x570bf1(0x15d)]('');
  const ra = sa[gdijsorg_0x570bf1(0x15a)]();
  const ja = ra[gdijsorg_0x570bf1(0x15e)]('');
  const aj = ja[gdijsorg_0x570bf1(0x159)](0x18)[gdijsorg_0x570bf1(0x155)](0x0, -0x14);
  return aj;
}
...
$(() => {
  ...
  render(path);
  // $(window).on('Adjust Iframes load', adjustIframes)
  injekAds();
  adjustIframes();
  setTimeout(reloadIFrame, 180000);
...
function adsBlocked(callback) {
  const iklanURL = 'https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js';
  ...
  const myReq = new Request(iklanURL, myInit);
...
// const chk = await checkAdBlocker()
// checkAdBlocker().then(response => {
//   'do-something'
// })
