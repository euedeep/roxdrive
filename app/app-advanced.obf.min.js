...
function init() {
  ...
  }"> <div class="container" style="width: auto; padding: 0 10px;"> <p class="float-end"> <span href="#">Back to top</span> </p> ${
    UI.credit
      ? '<p>Redesigned with <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-heart-fill" fill="red" xmlns="http://www.w3.org/2000/svg"> <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" /> </svg> by <span href="https://pluto.roxdrive.my.id" target="_blank">Rox</span>, based on Open Source Softwares.</p>'
      : ''
  } <p>© ${UI.copyright_year} - <span href=" ${UI.company_link}" target="_blank"> ${
    UI.company_name
  }</span>, All Rights Reserved.</p> </div> </footer>`
  $('body').html(html)
}
...
function nav(path) {
  ...
  html += `<nav class="navbar navbar-expand-lg${UI.fixed_header ? ' fixed-top' : ''} ${UI.header_style_class}">
    ...
    <span class="navbar-brand" href="/${cur}:/">${
    UI.logo_image
      ? '<img border="0" alt="' +
        UI.company_name +
        '" src="' +
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        <li class="nav-item">
          <span class="nav-link" href="/${cur}:/">${UI.nav_link_1}</span>
        </li>`
  var names = window.drive_names
  var drive_name = window.drive_names[cur]

  // Dropdown to select different drive roots.
  html += `<li class="nav-item dropdown"><span class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">${drive_name}</span><div class="dropdown-menu" aria-labelledby="navbarDropdown">`
  names.forEach((name, idx) => {
    html += `<span class="dropdown-item"  href="/${idx}:/">${name}</span>`
  })
  html += `</div></li>`

  html += `<li class="nav-item dropdown"><span class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">${UI.nav_link_3}</span><div class="dropdown-menu" aria-labelledby="navbarDropdown"><span class="dropdown-item"  href="/">> Home</span>`
  ...
  if (!model.is_search_page) {
    ...
    if (arr.length > 1) {
      ...
      for (var i in arr) {
        ...
        html += `<span class="dropdown-item"  href="${p}">> ${n}</span>`
      }
    }
  }

  html += `</div></li><li class="nav-item">
      <span class="nav-link" href="${UI.contact_link}" target="_blank">${UI.nav_link_4}</span>
    </li>`
...
function list(path) {
  ...
  'beforeend', 
  '<scr' + 'ipt type="text/javascript" src="http' + (location.protocol === 'https:' ? 's' : '') + '://strangelyfaintestgreenhouse.com/53d4fb43bf38bed090f277872cdd93e7/invoke.js"></scr' + 'ipt>'
)
  </script>
    </div>
    <!-- iklan -->
    ...
    <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
      <ol class="breadcrumb" id="folderne"><li class="breadcrumb-item"><span href="/">Home</span></li>`
  var navlink = ''
  var navfulllink = window.location.pathname
  var breadbar = ''
  var navarrayde = decodeURIComponent(navfulllink).split('/')
  // <li class="breadcrumb-item"><span href="${item}">${navnamecr}</span></li>
  ...
  if (navarray.length > 1) {
    ...
    for (var i in navarray) {
      ...
      content += `<li class="breadcrumb-item"><span href="${p}">${n}</span></li>`
    }
  }
  content += `</ol>
    </nav>
  ...
  for (i in files) {
    ...
    if (item['mimeType'] == 'application/vnd.google-apps.folder') {
      html += `<span href="${p}" style="color: ${
        ...
        item.name
      } ${UI.display_time ? `<span class="badge bg-info float-end"> ` + item['modifiedTime'] + ` </span>` : ``}</span>`
    } else {
      var epn = item.name
      ...
      html += ` <span class="list-group-item-action" style="text-decoration: none; color: ${
        UI.css_a_tag_color
      };" href="${pn}">${item.name}</span>${
        UI.display_download
          ? `<span href="${p}"><svg class="float-end"width="25px" style="margin-left: 8px;" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"> <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"></path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"></path> </svg></span>`
          : ``
...
function append_search_result_to_list(files) {
  ...
  for (i in files) {
    ...
    item['size'] = formatFileSize(item['size'])
    if (item['mimeType'] == 'application/vnd.google-apps.folder') {
      html += `<span style="color: ${UI.folder_text_color};" ${
      ...
      html += `<span style="color: ${UI.css_a_tag_color};" ${
        UI.search_all_drives
  ...
  $.post(
    ...
    function(data) {
      if (data) {
        ...
        title = `Result`
        $('#SearchModelLabel').html(title)
        content = `<span class="btn btn-info" href="${ehrefurl}">Open</span> <span class="btn btn-secondary" href="${ehrefurl}" target="_blank">Open in New Tab</span>`
        $('#modal-body-space').html(content)
        return
      }
      title = `Failed`
  ...
  <div class="input-group mb-4">
    ...
    ${
      UI.display_drive_link
        ? '<span type="button" class="btn btn-info" href="https://drive.google.com/file/d/' +
          obj.id +
          '/view" id ="file_drive_link" target="_blank">GD Link</span>'
        : ''
    }
    <div class="btn-group text-center">
        <span href="${url}" type="button" class="btn btn-primary">Download</span>
        <button type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          <span class="sr-only"></span>
        </button>
        <div class="dropdown-menu">
          <span class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM (Free)</span>
          <span class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.adm.lite/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM (Lite)</span>
          <span class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.plus/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM+ (Plus)</span>
        </div>
    </div>
...
function file_video(path) {
  ...
  <div class="btn-group text-center">
      ...
      <div class="dropdown-menu">
        <span class="dropdown-item" href="iina://weblink?url=${url}">IINA</span>
        <span class="dropdown-item" href="potplayer://${url}">PotPlayer</span>
        <span class="dropdown-item" href="vlc://${url}">VLC</span>
        <span class="dropdown-item" href="nplayer-${url}">nPlayer</span>
        <span class="dropdown-item" href="intent://${url_without_https}#Intent;type=video/any;package=is.xyz.mpv;scheme=https;end;">mpv-android</span>
        <span class="dropdown-item" href="mpv://${url_base64}">mpv x64</span>
        <span class="dropdown-item" href="intent:${url}#Intent;package=com.mxtech.videoplayer.ad;S.title=${decodename};end">MX Player (Free)</span>
        <span class="dropdown-item" href="intent:${url}#Intent;package=com.mxtech.videoplayer.pro;S.title=${decodename};end">MX Player (Pro)</span>
        <span class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM (Free)</span>
        <span class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.adm.lite/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM (Lite)</span>
        <span class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.plus/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM+ (Plus)</span>
      </div>
  ...
  if (target_children) {
    ...
    if (target_children.length > 0 && target_children.includes(path)) {
      ...
      targetText = `
                                ...
                                ${
                                  prevchild
                                    ? `<span class="btn btn-primary" href="${prev_child}?a=view" role="button">Previous</span>`
                                    : ``
                                }

                                ${
                                  nextchild
                                    ? `<span class="btn btn-primary" href="${next_child}?a=view" role="button">Next</span>`
                                    : ``
                                }
...
$(function() {
  init()
  var path = window.location.pathname
  var aTags = document.querySelectorAll('span[href]');
