...
function init() {
  ...
  <footer class="footer mt-auto py-3 text-muted ${UI.footer_style_class}" style="${UI.fixed_footer ? 'position: fixed; ' : ''}left: 0; bottom: 0; width: 100%; color: white; z-index: 9999;${
		...
		UI.credit ? '<p>Redesigned with <i class="fas fa-heart" style="color:#ff477e;"></i> by <a href="https://telegram.dog/codenrox" target="_blank">Rox</a>.</p>' : ''
...
function nav(path) {
	var originLok = window.location.origin + window.location.pathname + '#';
	var model = window.MODEL;
	var html = '';
	var cur = window.current_drive_order || 0;
	html += `<nav class="navbar navbar-expand-lg${UI.fixed_header ? ' fixed-top' : ''} ${UI.header_style_class}">
      <div class="container-fluid">
    <a class="navbar-brand py-0" href="/${cur}:/">${
	...
	html += `<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">${drive_name}<i class="fas fa-chevron-down fa-sm ps-1 transisi"></i></a><div class="dropdown-menu" aria-labelledby="navbarDropdown">`;
	names.forEach((name, idx) => {
		html += `<a class="dropdown-item" href="/${idx}:/">${name}</a>`;
	});
	html += `</div></li>`;

	html += `<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">${UI.nav_link_3}<i class="fas fa-chevron-down fa-sm ps-1 transisi"></i></a><div class="dropdown-menu" aria-labelledby="navbarDropdown"><a class="dropdown-item" href="/"><i class="fas fa-angle-right pe-2"></i>Home</a>`;
	...
	if (!model.is_search_page) {
		...
		if (arr.length > 1) {
			...
			for (var i in arr) {
				...
				html += `<a class="dropdown-item" href="${p}"><i class="fas fa-angle-right pe-2"></i>${n}</a>`;
			}
		}
	}
	...
	html += `</div></li><li class="nav-item">
      ...
      <a onclick="dialogPass()" class="nav-link" data-href="#">Password</a>
    </li>`;
  ...
  <form class="d-flex me-3" method="get" action="/${cur}:search">
  <div class="input-group input-group-sm bg-light-alt rounded">
  <input class="form-control bg-transparent border-1" name="q" type="search" placeholder="Search" aria-label="Search" aria-describedby="btn-cari" value="${search_text}" required>
  <button id="btn-cari" class="btn ${UI.search_button_class}" onclick="if($('#search_bar_form>input').val()) $('#search_bar_form').submit();" type="submit"><i class="fas fa-search"></i></button>
  </div>
  </form>
  </div>
  </div>
...
function list(path) {
	var content = `<div class="container">${UI.fixed_header ? '<br>' : ''}
  	<div id="update"></div>
      <!-- iklan atas -->
      <div id="bosque" class="mantul pb-3">
      <iframe id="kuwlFrame" src="" width="0" height="0" frameborder="0" scrolling="no"></iframe>
      </div>
      <!-- iklan atas -->
      <div id="head_md" style="display:none; padding: 20px 20px;"></div>
      <div class="${UI.path_nav_alert_class} d-flex align-items-center mb-0 pb-1 pt-1 ps-2" role="alert" style="/*! margin-bottom: 0; */ /*! padding-bottom: 0rem; */">
    <nav style="/*! --bs-breadcrumb-divider: '>'; */" aria-label="breadcrumb">
      <ol class="breadcrumb mb-0" id="folderne"><li class="breadcrumb-item"><a href="/">Home</a></li>`;
	var navlink = '';
	...
	content += `</ol>
      ...
      <div id="list" class="list-group text-break">
    	...
    	<div class="${UI.file_count_alert_class} text-center d-none text-dark" role="alert" id="count">Total <span class="number text-center text-dark"></span> items</div>
...
function append_files_to_list(path, files) {
	...
	for (i in files) {
		...
		if (item['mimeType'] == 'application/vnd.google-apps.folder') {
			...
			};" class="list-group-item list-group-item-action"><i style="color:#F6BB42;font-size:1.2rem;" class="fas fa-folder me-1"></i><span class="align-text-top">${item.name}</span>${
				UI.display_time ? '<span class="badge bg-info float-end p-1">' + item['modifiedTime'] + '</span>' : ''
			}</a>`;
		} else {
			var epn = item.name;
			...
			html += ` <a class="list-group-item-action text-body align-middle" style="text-decoration: none; /*color: ${UI.css_a_tag_color};*/" href="${pn}">${item.name}</a>${
				UI.display_download
					...
					: ''
			}${UI.display_size ? '<span class="badge bg-pink float-end p-1"> ' + item['size'] + ' </span>' : ''}${
				UI.display_time ? ' <span class="badge bg-info float-end p-1"> ' + item['modifiedTime'] + ' </span>' : ''
			}</div>`;
		}
	}
...
function render_search_result_list() {
    ...
    <div class="card">
    <div class="${UI.path_nav_alert_class} d-flex align-items-center mb-0" role="alert" style="/*! margin-bottom: 0; */">Search Results</div>
    <div id="list" class="list-group text-break">
    </div>
    </div>
    <div class="${UI.file_count_alert_class} text-center d-none text-dark" role="alert" id="count">Total <span class="number text-center text-dark"></span> items</div>
...
function append_search_result_to_list(files) {
	...
	for (i in files) {
		...
		if (item['mimeType'] == 'application/vnd.google-apps.folder') {
			...
			} class="list-group-item list-group-item-action text-body"><i style="color:#F6BB42;font-size:1.2rem;" class="fas fa-folder me-1"></i><span class="align-text-top">${item.name}</span>${
				UI.display_time ? '<span class="badge bg-info float-end p-1">' + item['modifiedTime'] + '</span>' : ''
			}</a>`;
		} else {
			var p = '/' + cur + ':/' + item.name;
			...
			html += `<a style="/*! color: ${UI.css_a_tag_color}; */" ${
				UI.search_all_drives
					...
					: 'onclick="onSearchResultItemClick(this)" data-bs-toggle="modal" data-bs-target="#SearchModel" id="' + item['id'] + '" teamDriveId="' + item['teamDriveId'] + '"'
			} gd-type="${item.mimeType}" class="list-group-item list-group-item-action text-body">`;
			...
			html += ` ${item.name}<span class="badge float-end csize"> ${UI.display_size ? '<span class="badge bg-pink float-end p-1"> ' + item['size'] + ' </span>' : ''}${
				UI.display_time ? ' <span class="badge bg-info float-end p-1"> ' + item['modifiedTime'] + ' </span>' : ''
			}</a>`;
		}
	}
...
function file_others(path) {
  ...
  <div class="card-body text-start">
    <div class="${UI.file_view_alert_class} text-dark" id="file_details" role="alert"><span class="text-warning ps-1 pe-1">Filename:</span>${
			obj.name
		}<br><span class="text-warning ps-1 pe-1">Size:</span>${size}</div>
  </div>
  <!-- iklan atas -->
  <div id="bosque" class="mantul pb-3">
    <iframe id="kuwlFrame" src="" width="0" height="0" frameborder="0" scrolling="no"></iframe>
  ...
  <div class="input-group mb-4">
    ...
    <div class="btn-group text-center">
        ...
        <button id="donlotother" type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-display="static" data-bs-auto-close="true" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		<i class="fas fa-angle-down fa-fade" aria-hidden="true"></i>
		<span class="sr-only"></span>
        </button>
        <div class="dropdown-menu" aria-labelledby="donlotother">
...
function file_code(path) {
	...
	$.post('', function(data) {
		...
		var size = formatFileSize(obj.size);
		var content = `
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/highlightjs/cdn-release@11.2.0/build/styles/atom-one-dark-reasonable.min.css">
  <div class="container"><br>
  <div class="card text-center">
  <div class="card-body text-start">
    <div class="${UI.file_view_alert_class} text-dark" id="file_details" role="alert"><span class="text-warning ps-1 pe-1">Filename:</span>${
  ...
  <div class="input-group mb-4">
    ...
    <div class="btn-group text-center">
        <a href="${url}" type="button" class="btn btn-primary">Download</a>
        <button id="donlotfilecode" type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-display="static" data-bs-auto-close="true" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		<i class="fas fa-angle-down fa-fade" aria-hidden="true"></i>
		<span class="sr-only"></span>
        </button>
        <div class="dropdown-menu" aria-labelledby="donlotfilecode">
          <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM (Free)</a>
          <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.adm.lite/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM (Lite)</a>
          <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.plus/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM+ (Plus)</a>
        </div>
...
function file_video(path) {
  ...
  <div class="btn-group text-center">
      <a href="${url}" type="button" class="btn btn-primary">Download</a>
      <button id="donlotvideo" type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-display="static" data-bs-auto-close="true" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
	  <i class="fas fa-angle-down fa-fade" aria-hidden="true"></i>
	  <span class="sr-only"></span>
      </button>
      <div class="dropdown-menu" aria-labelledby="donlotvideo">
...
function file_audio(path) {
  ...
  <div class="input-group mb-4">
    ...
    <div class="btn-group text-center">
        <a href="${url}" type="button" class="btn btn-primary">Download</a>
        <button id="donlotaudio" type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-display="static" data-bs-auto-close="true" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		<i class="fas fa-angle-down fa-fade" aria-hidden="true"></i>
		<span class="sr-only"></span>
        </button>
        <div class="dropdown-menu" aria-labelledby="donlotaudio">
          <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM (Free)</a>
          <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.adm.lite/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM (Lite)</a>
          <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.plus/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM+ (Plus)</a>
        </div>
...
function file_pdf(path) {
  ...
  <div class="input-group mb-4">
    ...
    <div class="btn-group text-center">
        <a href="${url}" type="button" class="btn btn-primary">Download</a>
        <button id="donlotpdf" type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-display="static" data-bs-auto-close="true" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		<i class="fas fa-angle-down fa-fade" aria-hidden="true"></i>
		<span class="sr-only"></span>
        </button>
        <div class="dropdown-menu" aria-labelledby="donlotpdf">
          <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM (Free)</a>
          <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.adm.lite/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM (Lite)</a>
          <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.plus/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM+ (Plus)</a>
        </div>
...
function file_image(path) {
  ...
  <div class="input-group mb-4">
    ...
    <div class="btn-group text-center">
        <a href="${url}" type="button" class="btn btn-primary">Download</a>
        <button id="donlotimage" type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-display="static" data-bs-auto-close="true" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		<i class="fas fa-angle-down fa-fade" aria-hidden="true"></i>
		<span class="sr-only"></span>
        </button>
        <div class="dropdown-menu" aria-labelledby="donlotimage">
          <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM (Free)</a>
          <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.adm.lite/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM (Lite)</a>
          <a class="dropdown-item" href="intent:${url}#Intent;component=idm.internet.download.manager.plus/idm.internet.download.manager.Downloader;S.title=${decodename};end">1DM+ (Plus)</a>
        </div>
...
function utc2delhi(utc_datetime) {
	...
	return (
		...
		'-' +
		year +
		'<i class="fas fa-ellipsis-v fa-xs pe-1 ps-1"></i>' +
...
function injekAds() {
	...
	if (mini) {
		...
		// $rem.remove()
		$('#kuwlFrame').attr({
			src: '//marinegruffexpecting.com/watchnew?key=c747be716129596ba5e7f519fbe27152',
			width: 320,
			height: 50,
		});
	} else if (medium) {
		$('#kuwlFrame').attr({
			src: '//marinegruffexpecting.com/watchnew?key=fe818d050b02c7c836cf99122f033764',
			width: 468,
			height: 60,
		});
	} else if (big) {
		$('#kuwlFrame').attr({
			src: '//marinegruffexpecting.com/watchnew?key=d7fb8ebfd2973fc6fc22b1d22f2d33ed',
			width: 728,
			height: 90,
		});
	} else {
...
function injekIklanBawah() {
	var len = $('script[src="//marinegruffexpecting.com/93535642da474b1fa567c29830789fb3/invoke.js"]').length;
	var sc = '<script async="async" data-cfasync="false" src="//marinegruffexpecting.com/93535642da474b1fa567c29830789fb3/invoke.js"></' + 'script>';
	try {
		if (len === 0) {
			$(sc).appendTo('body');
		}
	} catch (e) {
		console.log('Error load skrip: ' + e.message);
	}
}
...
function dialogPass() {
	var clippy = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" id="copy-clippy" aria-hidden="true" role="img" width="1em" height="1em" class="tombol-copay-clippy d-inline-block" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16">
	<path fill-rule="evenodd" d="M5.75 1a.75.75 0 0 0-.75.75v3c0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75v-3a.75.75 0 0 0-.75-.75h-4.5zm.75 3V2.5h3V4h-3zm-2.874-.467a.75.75 0 0 0-.752-1.298A1.75 1.75 0 0 0 2 3.75v9.5c0 .966.784 1.75 1.75 1.75h8.5A1.75 1.75 0 0 0 14 13.25v-9.5a1.75 1.75 0 0 0-.874-1.515a.75.75 0 1 0-.752 1.298a.25.25 0 0 1 .126.217v9.5a.25.25 0 0 1-.25.25h-8.5a.25.25 0 0 1-.25-.25v-9.5a.25.25 0 0 1 .126-.217z" fill="currentColor"/>
	</svg>`;
	var check = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" id="copy-check" aria-hidden="true" role="img" width="1em" height="1em" class="tombol-copay-sukses d-none" preserveAspectRatio="xMidYMid meet" viewBox="0 0 16 16">
	<path fill-rule="evenodd" d="M13.78 4.22a.75.75 0 0 1 0 1.06l-7.25 7.25a.75.75 0 0 1-1.06 0L2.22 9.28a.75.75 0 0 1 1.06-1.06L6 10.94l6.72-6.72a.75.75 0 0 1 1.06 0z" fill="currentColor"/>
	</svg>`;
	var isMobile = 'ontouchstart' in window || 'msmaxtouchpoints' in window.navigator || /iphone|ipod|ipad|android|iemobile|iphone|ipad|ipod|blackberry|bada|mobi/i.test(navigator.userAgent);

	Swal.fire({
		title: '',
		html: `<div class="position-relative top-0 end-0">
		<pre id="targetCopy" class="highlight position-relative p-1 mb-0 top-0 end-0 pass-area">roxdrive</pre>		
		<button id="tombol-copy" class="tombol-copay position-absolute top-0 end-0 p-0" onclick="copy2Clipboard()" data-clipboard-target="#targetCopy" data-bs-toggle="tooltip" data-bs-original-title="" data-bs-placement="top">
		${clippy}
		${check}
		</button></div>`,
		...
		focusConfirm: false,
		confirmButtonText: '<i class="fas fa-thumbs-up fa-lg" style="color:#fff"></i>',
		confirmButtonAriaLabel: 'Thumbs up, great!',
		cancelButtonText: '<i class="fas fa-thumbs-down" style="color:#fff"></i>',
		cancelButtonAriaLabel: 'Thumbs down',
	});

	$('button[id="tombol-copy"]').tooltip({
		trigger: 'click',
		placement: 'top',
	});
	...
	if (isMobile) {
		$('pre.highlight')
			.nextAll('.tombol-copay')
			.addClass('d-inline-block');
		//console.info('User Mobile');
	} else {
		if (
			$('pre.highlight')
				.nextAll('.tombol-copay')
...
/**
 * @param {string} klas - element class yang akan diganti
 * @param {string} sebelum - Fontawesome icon sebelum, contoh: fa-thumbs-up.
 * @param {string} sesudah - Fontawesome icon pengganti sebelum, contoh: fa-thumbs-down. tanpa prefix fas
 */
function gantiIcon(klas, sesudah) {
	var ele = $(`${klas}`);
	var sebelum = new RegExp(/.*[\ \"\']+(fa-[a-zA-z0-9]*).*/);
	if (ele.hasClass(sebelum[1])) {
		ele.removeClass(sebelum[1]).addClass(sesudah);
	} else {
		ele.addClass(sebelum[1]).removeClass(sesudah);
	}
}
/**
 * ECMA2016 / ES6
...
// Anti-Aliasing @font-face
//function antiAliasingFont() {
const shadowify = function(e) {
	const color = $(e).css('color');
	const size = $(e).css('font-size');
	let hex;
	let ext;
	// get css hex color value
	if (color.indexOf('#') > -1) {
		//for IE
		hex = color;
	} else {
	...
	//if (color.toLowerCase().indexOf('rgb') == -1) return;

	let conv = hex.replace('#', '');
	if (conv.length === 3) {
		conv = `${hex[0]}${hex[0]}${hex[1]}${hex[1]}${hex[2]}${hex[2]}`;
	}
	const r = parseInt(conv.substring(0, 2), 16);
	const g = parseInt(conv.substring(2, 4), 16);
	const b = parseInt(conv.substring(4, 6), 16);

	//var rgba = color.match(/^rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*(\d+))?\)$/);
	//$(e).css('text-shadow', '0 0 1px rgba(' + rgba[1] + ',' + rgba[2] + ',' + rgba[3] + ',0.2)');

	// To use calculated shadow of say, 1/15th of the font height
	//const fsize = size.match(/(\d+)px/);
	//const fsize = size.match(/(\.|)(\d+)(px|em|rem|%)/g);
	const px = size.match(/(\d+)px/);
	const em = size.match(/([^\.\d+]\d+)em/g);
	const rem = size.match(/([^\.\d+]\d+)rem/g);
	const emt = size.match(/(\.)+(\d+)em/g);
	const remt = size.match(/(\.)+(\d+)rem/g);
	if (remt) {
		ext = '0rem 0rem .' + remt[2] / 30 + 'rem';
		console.info('remt');
	} else if (em) {
		ext = '0em 0em ' + em[1] / 30 + 'em';
		console.info('em');
	} else if (rem) {
		ext = '0rem 0rem ' + rem[1] / 30 + 'rem';
		console.info('rem');
	} else if (emt) {
		ext = '0em 0em .' + emt[2] / 30 + 'em';
		console.info('emt');
	} else if (px) {
		ext = '0px 0px ' + px[1] / 30 + 'px';
		console.info('px');
	}
	//$(e).css('text-shadow', '0 0 ' + fsize[1] / 30 + 'px rgba(' + rgba[1] + ',' + rgba[2] + ',' + rgba[3] + ',0.2)');
	$(e).css('text-shadow', `0 0 ${ext} rgba(${r},${g},${b},0.2)`);
};
...
$(function() {
	...
	var sweetalert = `<script async="async" src="https://cdn.jsdelivr.net/npm/sweetalert2@11.1.2/dist/sweetalert2.min.js"></script>
	<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@sweetalert2/themes@5.0.2/wordpress-admin/wordpress-admin.min.css">`;
	...
	// $('body, a, p, .input-group, span, #file_details, ul li, .navbar').each(function() {
	// 	shadowify(this);
	// });
	$('body, a, p, .input-group, span, #file_details, ul li, .navbar').each(function() {
		shadowify(this);
	...
	aTags.forEach(link => {
		link.addEventListener('click', function(e) {
			var ele = e.target;
			window.location.href = ele.getAttribute('data-href');
	...
	// nav link animasi
	$('a[id="navbarDropdown"]').click(function() {
		if (
			$(this)
				.find($('.fas'))
				.hasClass('fa-chevron-down')
		) {
			$('div[aria-labelledby="navbarDropdown"]').slideUp(200); //slide up all the sub divs
			$('a[id="navbarDropdown"]')
				.find($('.fas'))
				.removeClass('fa-rotate-180 pe-1'); // bring all iconsto normal
			$(this)
				.find($('.fas'))
				.addClass('fa-rotate-180 pe-1'); // rotate icon by 180
			if (
				$(this)
					.next()
					.is(':visible')
				...
				$('a[id="navbarDropdown"]')
					.find($('.fas'))
					.removeClass('fa-rotate-180 pe-1'); // if yes then undo icon rotation
				$(this)
					.next()
					.slideUp(200); // slide current up
			} else
				$(this)
					.next()
					.slideDown(200); // else slide the current down
		}
	});

	$(document).click(function(e) {
		if (!$('a[id="navbarDropdown"]').is(e.target) && !$('a[id="navbarDropdown"] i').is(e.target)) {
			//if clicked other than 'a' or 'i' tag
			$('div[aria-labelledby="navbarDropdown"]').slideUp(200); // slide all up
			if (
				$('a[id="navbarDropdown"]')
					.find($('.fas'))
					.hasClass('fa-rotate-180 pe-1')
			) {
				$(this)
					.find($('.fas'))
					.removeClass('fa-rotate-180 pe-1'); //remove all rotation
			}
		}
	});
	//$('body').attr('onload', 'siIoE4aX();');
