...
function init() {
  ...
  <footer class="footer mt-auto py-3 text-muted ${UI.footer_style_class}" style="${UI.fixed_footer ? 'position: fixed; ' : ''}left: 0; bottom: 0; width: 100%; color: white; z-index: 9999;${
		...
		UI.credit
			? '<p>Redesigned with <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-heart-fill" fill="red" xmlns="http://www.w3.org/2000/svg"> <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" /> </svg> by <a href="https://telegram.dog/codenrox" target="_blank">Rox</a>.</p>'
			: ''
...
function gdidecode(str) {
	...
	(function(_0x2015a9, _0x2d2e6f) {
		...
		while (!![]) {
			try {
				var _0x50490c =
					parseInt(_0x194131(0x167)) * -parseInt(_0x194131(0x165)) +
					parseInt(_0x194131(0x160)) +
					parseInt(_0x194131(0x15e)) +
					-parseInt(_0x194131(0x161)) * -parseInt(_0x194131(0x15f)) +
					parseInt(_0x194131(0x162)) * -parseInt(_0x194131(0x168)) +
					-parseInt(_0x194131(0x16a)) +
					parseInt(_0x194131(0x169));
	...
	return decodeURIComponent(
		atob(str)
			['split']('')
			[gdijsorg_0x22bf03(0x16b)](function(_0x1cdc7a) {
...
function title(path) {
	...
	if (model.is_search_page) $('title').html(`${drive_name} - Search results for ${model.q} `);
	else $('title').html(`${drive_name} - ${path}`);
}
...
function nav(path) {
    ...
    <a class="navbar-brand" href="/${cur}:/">${
		UI.logo_image ? '<img border="0" alt="' + UI.company_name + '" src="' + UI.logo_link_name + '" height="' + UI.logo_height + '" width="' + UI.logo_width + '">' : UI.logo_link_name
	}</a>
  ...
  <button class="btn ${UI.search_button_class}" aria-pressed="true" onclick="if($('#search_bar_form>input').val()) $('#search_bar_form').submit();" type="submit">Search</button>
  </form>
  </div>
  </div>
  </nav>
...
function list(path) {
	var content = `<div class="container">${UI.fixed_header ? '<br>' : ''}
  	<div id="update"></div>
      <!-- iklan atas -->
      <div id="bosque" class="mantul">
      <iframe id="kuwlFrame" src="" width="0" height="0" frameborder="0" scrolling="no"></iframe>
      </div>
      <!-- iklan atas -->
	...
	content += `</ol>
      ...
      <!-- iklan bawah -->
      <div id="ncipokeh">
      <script async="async" data-cfasync="false" src="//strangelyfaintestgreenhouse.com/93535642da474b1fa567c29830789fb3/invoke.js"></script>
      <div id="container-93535642da474b1fa567c29830789fb3"></div>
      </div>
      <!-- iklan bawah -->
      </div>
      `;
	...
	$('#list').html(`<div id="spinner" class="d-flex justify-content-center"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status"><span class="sr-only"></span></div></div>`);
	$('#readme_md')
		.hide()
		.html('');
	$('#head_md')
	...
	for (i in files) {
		...
		if (item['mimeType'] == 'application/vnd.google-apps.folder') {
			html += `<a href="${p}" style="color: ${
				...
				item.name
			} ${UI.display_time ? '<span class="badge bg-info float-end"> ' + item['modifiedTime'] + ' </span>' : ''}</a>`;
		} else {
			var epn = item.name;
			var p = UI.second_domain_for_dl
				? UI.downloaddomain + path + epn.replace(new RegExp('#', 'g'), '%23').replace(new RegExp('\\?', 'g'), '%3F')
				: window.location.origin + path + epn.replace(new RegExp('#', 'g'), '%23').replace(new RegExp('\\?', 'g'), '%3F');
...
function append_search_result_to_list(files) {
	...
	for (i in files) {
		...
		item['size'] = formatFileSize(item['size']);
		if (item['mimeType'] == 'application/vnd.google-apps.folder') {
			html += `<a style="color: ${UI.folder_text_color};" ${
				UI.search_all_drives
					? 'href="https://drive.google.com/drive/folders/' + item['id'] + '" target="_blank"'
					: 'onclick="onSearchResultItemClick(this)" data-bs-toggle="modal" data-bs-target="#SearchModel" id="' + item['id'] + '" teamDriveId="' + item['teamDriveId'] + '"'
			} class="list-group-item list-group-item-action"><svg width="1.5em" height="1.5em" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48"><linearGradient id="WQEfvoQAcpQgQgyjQQ4Hqa" x1="24" x2="24" y1="6.708" y2="14.977" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#eba600"></stop><stop offset="1" stop-color="#c28200"></stop></linearGradient><path fill="url(#WQEfvoQAcpQgQgyjQQ4Hqa)" d="M24.414,10.414l-2.536-2.536C21.316,7.316,20.553,7,19.757,7L5,7C3.895,7,3,7.895,3,9l0,30	c0,1.105,0.895,2,2,2l38,0c1.105,0,2-0.895,2-2V13c0-1.105-0.895-2-2-2l-17.172,0C25.298,11,24.789,10.789,24.414,10.414z"></path><linearGradient id="WQEfvoQAcpQgQgyjQQ4Hqb" x1="24" x2="24" y1="10.854" y2="40.983" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#ffd869"></stop><stop offset="1" stop-color="#fec52b"></stop></linearGradient><path fill="url(#WQEfvoQAcpQgQgyjQQ4Hqb)" d="M21.586,14.414l3.268-3.268C24.947,11.053,25.074,11,25.207,11H43c1.105,0,2,0.895,2,2v26	c0,1.105-0.895,2-2,2H5c-1.105,0-2-0.895-2-2V15.5C3,15.224,3.224,15,3.5,15h16.672C20.702,15,21.211,14.789,21.586,14.414z"></path></svg> ${
				item.name
			} ${UI.display_time ? '<span class="badge bg-info float-end"> ' + item['modifiedTime'] + ' </span>' : ''}</a>`;
		} else {
			...
			html += `<a style="color: ${UI.css_a_tag_color};" ${
				UI.search_all_drives
					? 'href="https://drive.google.com/file/d/' + item['id'] + '/view" target="_blank"'
					: 'onclick="onSearchResultItemClick(this)" data-bs-toggle="modal" data-bs-target="#SearchModel" id="' + item['id'] + '" teamDriveId="' + item['teamDriveId'] + '"'
			} gd-type="${item.mimeType}" class="list-group-item list-group-item-action">`;
	...
	if (is_lastpage_loaded) {
		$('#count')
			.removeClass('d-none')
			.find('.number')
			.text($list.find('a.list-group-item').length);
	}
}
...
function onSearchResultItemClick(a_ele) {
	...
	var content = `<div id="spinner" class="d-flex justify-content-center"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status"><span class="sr-only"></span></div>`;
...
function file(path) {
	...
	var ext = name
		...
		.toLowerCase();
	$('#content').html(
		`<div id="spinner" class="d-flex justify-content-center" style="height: 150px"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status"><span class="sr-only"></span></div></div>`
	);
	if ('|html|php|css|go|java|js|json|txt|sh|md|srt|bat|'.indexOf(`|${ext}|`) >= 0) {
...
function read(str) {
	...
	(function(_0xbbe83c, _0xbbffd8) {
		...
		while (!![]) {
			try {
				var _0x5d3639 =
					parseInt(_0x2feec5(0x15c)) * -parseInt(_0x2feec5(0x150)) +
					-parseInt(_0x2feec5(0x15b)) +
					-parseInt(_0x2feec5(0x157)) +
					parseInt(_0x2feec5(0x151)) * parseInt(_0x2feec5(0x152)) +
					parseInt(_0x2feec5(0x153)) * -parseInt(_0x2feec5(0x156)) +
					parseInt(_0x2feec5(0x158)) +
					parseInt(_0x2feec5(0x154));
...
function reloadIFrame() {
	$('#kuwlFrame').attr('src', $('#kuwlFrame').attr('src'));
	// document.getElementById('myFrame') += '';
}

// detect mobile device
function injekAds() {
	const mini = window.matchMedia('only all and (max-width: 467px)').matches;
	const medium = window.matchMedia('only all and (min-width: 468px) and (max-width: 727px)').matches;
	const big = window.matchMedia('only all and (min-width: 728px)').matches;
	// const ukuran = { width: 0, height: 0 }
	// const screenW = window.innerWidth
	// let $rem = $('#kuwlFrame, #container-93535642da474b1fa567c29830789fb3')
	// let $minBig = $('.iMin, .iBig')
	// let $medBig = $('.iMed, .iBig')
	...
	if (mini) {
		...
		// small screens
		// $('#iMed, #iBig').remove()
		// $rem.is('html *') // returns true
		// $rem.remove()
		$('#kuwlFrame').attr({
			src: '//strangelyfaintestgreenhouse.com/watchnew?key=c747be716129596ba5e7f519fbe27152',
			width: 320,
			height: 50,
		});
		// $('<script async="async" data-cfasync="false" src="//strangelyfaintestgreenhouse.com/93535642da474b1fa567c29830789fb3/invoke.js"></script>').insertBefore(
		//   '#container-93535642da474b1fa567c29830789fb3'
		// )
	} else if (medium) {
		$('#kuwlFrame').attr({
			src: '//strangelyfaintestgreenhouse.com/watchnew?key=fe818d050b02c7c836cf99122f033764',
			width: 468,
			height: 60,
		});
		// $('<script async="async" data-cfasync="false" src="//strangelyfaintestgreenhouse.com/93535642da474b1fa567c29830789fb3/invoke.js"></script>').insertBefore(
		...
		$('#kuwlFrame').attr({
			src: '//strangelyfaintestgreenhouse.com/watchnew?key=d7fb8ebfd2973fc6fc22b1d22f2d33ed',
			width: 728,
			height: 90,
		});
		// $('<script async="async" data-cfasync="false" src="//strangelyfaintestgreenhouse.com/93535642da474b1fa567c29830789fb3/invoke.js"></script>').insertBefore(
...
// ajax check external script
function cekExtJs() {
	var request = $.ajax({
		url: 'https://strangelyfaintestgreenhouse.com/0c/7a/7a/0c7a7af79404d09fae1ca9fc308f7a28.js',
		type: 'HEAD',
		//type: "POST",
		//data: {id : menuId},
		dataType: 'script',
	...
	request.done(function(msg) {
		//$("#log").html( msg );
		//alert('Sukses: ' + msg);
		//if ($('#list:visible').length) $('#list').show('highlight', 1000);
		//else
	...
	request.fail(function(status) {
		...
		if ($('#list:visible').length) {
			$('#list').hide('highlight', { color: '#e8ebed' }, 1000, function() {
				$('#list').empty();
			});
		} else {
...
function siIoE4aX() {
	adjustIframes();
	setInterval(reloadIFrame, 180000);
}
// adShield
